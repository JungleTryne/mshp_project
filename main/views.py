...
def add_snippet_page(request):
    ...
    :param request: объект c деталями запроса
    :type request: :class:`django.http.HttpRequest`
    :return: объект ответа сервера с HTML-кодом внутри в случае, если идёт \
...
def delete_snippet_page(request, snippet_id):
    ...
    :raises: :class:`django.http.Http404` в случае, если сниппет с \
        указанным ID не существует
    :raises: :class:`django.http.Http404` в случае, если сниппет с \
        указанным ID не принадлежит пользователю
    :return: перенаправление на страницу списка сниппетов
    :return: перенаправление на страницу логина в случае неавторизованного \
        пользователя
    """
