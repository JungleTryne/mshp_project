...
class BaseFormatter:
    ...
    Идея работы проста: сначала создаётся объект форматтера, потом \
        вызывается метод `get_formatted_code`.
    Он проверяет существование файла с отформатированным кодом.
    Если файл есть - данные берутся из него, если его нет - запускается \
        утилита и данные записываются в файл.
    ...
    def get_formatted_code_name(self):
        ...
        В случае, если утилита не указана, используется имя \
            оригинального файла,
    ...
    def save_formatted_code_to_file(self):
        """
        Сохранение отформатированного утилитой кода в файл. Заготовка \
    ...
    def get_formatted_code(self):
        """
        Получение форматированного кода на основе имеющегося имени файла \
...
class CommandLineFormatter(BaseFormatter):
    ...
    :param OPTS: параметры запуска утилиты (все, кроме имени \
    ...
    def __init__(self, filename):
        ...
        :raises: :class:`ModuleNotFoundError` в случае, если утилита не \
            установлена.
        """
...
class DocFormatter(CommandLineFormatter):
    ...
    docformatter - утилита, приводящая doc-строки в соответствие с \
        рекомендациям PEP257
    """
...
class AutoFlakeFormatter(CommandLineFormatter):
    ...
    autoflake - утилита, чистящая код от неиспользуемых импортов \
        и переменных
    """
    UTILITY = 'autoflake'
